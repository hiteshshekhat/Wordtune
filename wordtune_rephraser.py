...
def rephraser(docs_url):
    ...
    email = WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.XPATH, '//input[@name="email"]')))
    email.send_keys(my_email)
    password = WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.XPATH, '//input[@name="password"]')))
    password.send_keys(my_password)
    login_button = WebDriverWait(driver, 10).until(
        EC.presence_of_element_located((By.XPATH, '//button[@data-testid="login_button"]'))
    )
    login_button.click()
    sleep(5)
    driver.get("https://app.wordtune.com/editor")
    try:
        welcome_window = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.XPATH, "//body/div[3]/div[3]/div/div/div/div[1]/div[4]"))
        )
    except TimeoutException as e:
        welcome_window = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.XPATH, "//body/div[4]/div[3]/div/div/div/div[1]/div[4]"))
        )
    welcome_window.click()
    editor = WebDriverWait(driver, 10).until(
        EC.presence_of_element_located((By.XPATH, '//div[@id="editorContentEditable"]'))
    )
    ...
    js = """var sel = window.getSelection ? window.getSelection() : document.selection;
        if (sel) {
    ...
    for child in child_elements:
        driver.execute_script('arguments[0].scrollIntoView({block: "center", inline: "center"})', child)
        driver.execute_script(
            "if (document.body.createTextRange) {const range = document.body.createTextRange();range.moveToElementText(arguments[0]);range.select();} else if (window.getSelection) {const selection = window.getSelection();const range = document.createRange();range.selectNodeContents(arguments[0]);selection.removeAllRanges();selection.addRange(range);}",
            child,
        )
        rewrite_button = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.XPATH, '//button[@data-testid="editorToolbarButtonRewrite"]'))
        )
        ...
        for i in range(100000):
            try:
                first_div = WebDriverWait(driver, 10).until(
                    EC.presence_of_element_located((By.XPATH, '//div[@data-testid="RewriteBox"]/div[1]/div[1]'))
                )
                first_div.click()
                sleep(1)
            except TimeoutException as e:
